...
def backend_correctness_checks(report: Report, loaded, tables, args) -> None:
    ...
    def run_pair(decode_name: str, prefill_name: str, graphs):
        engine = ContinuousBatchingEngine(
            ...
            num_blocks=args.num_blocks, block_size=args.block_size, max_active=4,
        ...
        if graphs:
            ...
            # A backend may declare itself capture-unsafe (FA2's kvcache path does on
            # some builds). That is a property to report, not a missing-graph failure:
            # the engine must then run *that phase* eagerly and keep the other phase's
            # graphs, which is what is asserted here.
            if not engine.decode_backend.graph_safe:
                if engine._decode_graphs:
                    raise AssertionError("an eager-only decode backend captured graphs")
                note = "decode eager-only by backend declaration; "
            elif not engine._decode_graphs:
                raise AssertionError("no decode graph was captured")
            if not engine.prefill_backend.graph_safe:
                if engine._prefill_graphs:
                    raise AssertionError("an eager-only prefill backend captured graphs")
                note += "prefill eager-only by backend declaration; "
        ...
        return f"{note}divergence vs stock: {divergence}"
...
def main() -> int:
    ...
    parser.add_argument("--block-size", type=int, default=16,
                        help="KV page size. FlashAttention's paged API needs 256; the "
                             "backend table reports which backends the choice excludes")
